...
void file_blocks()
{
    ...
    mpz_inits(F, f, b_size, temp, nn, NULL);
    ...
    while (i != str.length())
    {
        if (mpz_cmp_ui(b_size, s.length()) == 0)
        {
            ...
            mpz_mod(blocks[t1], blocks[t1], nn);
            t1++;
            s.clear();
        }
        s += str[i];
    ...
    ofstream out("file_blocks_out.txt");
    // cout<<"index\tblock\n";
    for (i = 0; i < mpz_get_ui(f); i++)
    {
        // cout<<i<<"\t";
        gmp_printf("%d %Zd\n", i, blocks[i]);
        char *tmp = mpz_get_str(NULL, 10, blocks[i]);
        out << i << " " << tmp << endl;
    }
    out.close();
...
void read_n()
{
    ...
    mpz_set_str(nn, n_keygen.c_str(), 10);
}
