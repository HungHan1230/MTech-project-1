...
void keygen()
{
    ...
    ofstream out("key_gen_out.txt");
    char *tmp_n = mpz_get_str(NULL, 10, n);
    char *tmp_e = mpz_get_str(NULL, 10, e);
    char *tmp_d = mpz_get_str(NULL, 10, d);
    char *tmp_v = mpz_get_str(NULL, 10, v);
    char *tmp_iv_temp = mpz_get_str(NULL, 10, iv_temp);
    // out << "n -> %Zd\ne -> %Zd\nd -> %Zd\nv -> %Zd\niv_temp -> %Zd\n", n, e, d, v, iv_temp;
    out << "n -> " << tmp_n << endl;
    out << "e -> " << tmp_e << endl;
    out << "d -> " << tmp_d << endl;
    out << "v -> " << tmp_v << endl;
    out << "iv_temp -> " << tmp_iv_temp << endl;
    out.close();
}
